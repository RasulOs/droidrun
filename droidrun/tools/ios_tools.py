...
class IOSTools:
    ...
    def __init__(
        ...
        self.last_tapped_rect: Optional[str] = None  # Store last tapped element's rect for text input
        logger.info(f"iOS device URL: {serial}")

    async def get_clickables(self, serial: Optional[str] = None) -> List[Dict[str, Any]]:
        """
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(a11y_url) as response:
                    if response.status == 200:
                        a11y_data = await response.text()
                        
                        # Parse the iOS accessibility tree format
                        elements = self._parse_ios_accessibility_tree(a11y_data)
                        ...
                        logger.error(f"Failed to get accessibility data: HTTP {response.status}")
                        raise ValueError(f"Failed to get accessibility data: HTTP {response.status}")
    ...
    def _parse_ios_accessibility_tree(self, a11y_data: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if not line.strip() or line.startswith('Attributes:') or line.startswith('Element subtree:') or line.startswith('Path to element:') or line.startswith('Query chain:'):
                continue
                
            # Parse UI elements - look for lines with coordinates
            # Format: ElementType, {{x, y}, {width, height}}, [optional properties]
            coord_match = re.search(r'\{\{([0-9.]+),\s*([0-9.]+)\},\s*\{([0-9.]+),\s*([0-9.]+)\}\}', line)
            ...
            if coord_match:
                ...
                element_type = element_type_match.group(1).strip() if element_type_match else 'Unknown'
                ...
                interactive_types = ['Button', 'SearchField', 'TextField', 'Cell', 'Switch', 'Slider', 'Stepper', 'Picker', 'Link']
                if any(interactive_type in element_type for interactive_type in interactive_types):
    ...
    async def tap_by_index(self, index: int, serial: Optional[str] = None) -> str:
        ...
        try:
            ...
            if not element:
                ...
                indices = [elem.get('index') for elem in self.clickable_elements_cache if elem.get('index') is not None]
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(swipe_url, json=payload) as response:
                    if response.status == 200:
                        logger.info(f"Swiped from ({start_x}, {start_y}) to ({end_x}, {end_y}) direction: {direction}")
                        return True
                    else:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(screenshot_url) as response:
                    if response.status == 200:
                        ...
                        logger.info(f"Screenshot captured successfully, size: {len(screenshot_data)} bytes")
                        return ("PNG", screenshot_data)
                    else:
                        logger.error(f"Failed to capture screenshot: HTTP {response.status}")
                        raise ValueError(f"Failed to capture screenshot: HTTP {response.status}")
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(a11y_url) as response:
                    if response.status == 200:
                        ...
                        # Analyze the accessibility data to determine state
                        is_keyboard_shown = "keyboard" in a11y_data.lower() or "textfield" in a11y_data.lower()
                        
                        return {
                            "current_activity": "iOS App (activity info not available)",
                            "keyboard_shown": is_keyboard_shown,
                            "accessibility_elements_count": len(a11y_data.split('\n')) if a11y_data else 0
                        }
                    else:
                        return {
